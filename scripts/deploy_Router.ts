...
async function main() {
  ...
  const factoryAddress = process.env.MAINNET_FACTORY_ADDRESS || '';
  const wethAddress = process.env.MAINNET_WETH_ADDRESS || '';
  ...
  const factory =  await deploy("UniswapV2Router02", [factoryAddress, wethAddress]);
