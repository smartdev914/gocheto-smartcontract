...
const deploy = async (contractName: string, _args: any[] = [], overrides = {}) => {
  console.log(` 🛰  Deploying: ${contractName}`);

  const contractArgs: any = _args || [];
  const stringifiedArgs = JSON.stringify(contractArgs);
  const contractFactory = await ethers.getContractFactory(contractName);
  const contract = await upgrades.deployProxy(contractFactory, contractArgs, overrides)
...
async function main() {
  ...
  const usdt =  await deploy("MockToken", ["Tether Token", "USDT"]);
  contracts.push(usdt);

  const usdc =  await deploy("MockToken", ["USD Coin", "USDC"]);
  contracts.push(usdc);

  const dai =  await deploy("MockToken", ["Maker DAO", "DAI"]);
  contracts.push(dai);
